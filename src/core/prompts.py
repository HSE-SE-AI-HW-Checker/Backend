...
def get_orchestrator_prompt(requirements: str) -> str:
    """
    ...
    return f"""
Ты - технический лид, распределяющий задачи по аудиту кода.
...
Важно: Каждое требование должно быть назначено ровно одному специалисту. Если требование затрагивает несколько областей, выбери самого релевантного специалиста.

Формат вывода (JSON):
Ты должен возвращать JSON объект, где ключом является порядковый номер требования, а значением - назначенный специалист.
